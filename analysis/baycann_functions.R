...
prepare_data <- function(xtrain, ytrain, xtest, ytest, scale_type){
  ...
  yresults <- scale_data(y,scale_type)  
...
scale_data  <- function(unscaled_data,type){
  ...
  if (type==4) {
    scaled_data <- (unscaled_data - mat.q50) / (mat.q75 - mat.q25)              ### Robust standardized
  }
  
  if (type==5) {
    scaled_data <- (unscaled_data - mat.means) / (mat.maxs - mat.mins)          ### Mean normalized
  }
  
  if (type==6) {
    scaled_data <- (unscaled_data) / (mat.dist)                                 ###Euclidean distance 
  }
  
  if (type==7) {
    scaled_data <- apply(unscaled_data, 2, box_cox_norm)                          ### Box-Cox Normalization
  }
  
  if (type==8) {
    scaled_data <- apply(unscaled_data, 2, best_normal)                          ### Best Normalization
  }
...
box_cox_norm <- function(vec) {
  
  b <- boxcox(lm(vec ~ 1))
  lambda <- b$x[which.max(b$y)] # Exact lambda that maximize the log-Likelihood
