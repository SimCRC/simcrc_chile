...
#Check that l_ANN_settings and calibration_settings exist:
if(!exists("l_ANN_settings")){
  stop("l_ANN_settings not found. Please run the R/04_BayCANN_setup.R script to create it.")
}
if(!exists("paths_calibration")){
  stop("paths_calibration not found. Please run the analysis/01_calibration_setup.R script to create it.")
}

#### 2. General parameters ========================================================

###### 2.1 parameters for Data preparation 
scale_type = l_ANN_settings$scale_type  ## 1: for scale from -1 to 1; 2: for standardization ; 3: for scale from 0 to 1
set.seed(1234)
...
data_true_targets  <- read.csv(paths_calibration$path_targets)
...
#Get and save priors
l_priors <- calibration_setup$params_priors
...
selected_targets <- calibration_setup$selected_targets
...
df_samp_post_ann   <- reshape2::melt(cbind(Distribution = "Posterior SimCRC",
